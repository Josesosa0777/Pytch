...
class TestLastView(BuildBatch):
  ...
  def test_last_view(self):
    view = self.batch.create_table_from_last_entries()
...
class TestDayLimit(BuildBatch):
  ...
  def test_date(self):
    view = self.batch.create_table_from_last_entries(date='2013-09-13')
  ...
  def test_start_date(self):
    view = self.batch.create_table_from_last_entries(start_date='2013-09-14')
    result = [meas for meas, in self.batch.query("""
      SELECT measurements.basename FROM %s en
        JOIN measurements ON measurements.id = en.measurementid
      """ % view)]
  ...
  def test_end_date(self):
    view = self.batch.create_table_from_last_entries(end_date='2013-09-14')
    result = [meas for meas, in self.batch.query("""
      SELECT measurements.basename FROM %s en
        JOIN measurements ON measurements.id = en.measurementid
      """ % view)]
  ...
  def test_between_dates(self):
    view = self.batch.create_table_from_last_entries(start_date='2013-09-13',
                                                  end_date='2013-09-14')
    result = [meas for meas, in self.batch.query("""
      SELECT measurements.basename FROM %s en
        JOIN measurements ON measurements.id = en.measurementid
