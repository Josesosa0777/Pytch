...
class Search(interface.iSearch):
    ...
    def fill(self, group):
        ...
        votes = self.batch.get_labelgroups("engine running")
        report = Report(cIntervalList(time), "Engine running", votes=votes)

        if not np.any(np.diff(time) > 100):
            # add intervals
            intervals = {
                'n/a': n_engine >= 8128.0,  # 0xFE00 * 0.125 = error
            ...
            for vote, mask in intervals.iteritems():
                ...
                for st, end in intvals:
                    ...
                    report.vote(index, 'engine running', vote)
        else:
