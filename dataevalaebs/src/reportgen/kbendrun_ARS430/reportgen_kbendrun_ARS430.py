...
class AebsAnalyze(Analyze):
    warningtemplate = 'Comparing'
    optdep = dict(
        #aebs_events='analyze_events_merged-last_entries@aebseval',
        FLR20aebs_events='analyze_events_FLR20-last_entries@aebseval',
        ARS430aebs_events='analyze_events_ARS430-last_entries@aebseval',
    ...
    def fill(self):
        ...
        story = intro(
            ...
            """
            This is an automatically generated report, based on field tests
            with simultaneously measured AC100/FLC21 AEBS and ARS430 AEBS.
            Both AEBS systems shall be evaluated and compared.<br/>
            <br/>
            The output signals of AEBS are analyzed and the relevant events 
            are collected in this report.<br/>
    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            story += [Paragraph(italic('Remark: Percentage values with "ca." are '
                      ...
                      Spacer(width=1 * cm, height=0.4 * cm),
                      Paragraph(bold('AC100, FLC21:')),]
            ...
            # AEBS warning rate for AC100
            if (self.optdep['FLR20aebs_events'] in self.passed_optdep and
                        self.optdep['count_vs_aebs_severity'] in self.passed_optdep):
                aebs_ei_ids = index_fill(self.modules.fill(self.optdep['FLR20aebs_events']))
                ...
                story += [Paragraph('Total number of AEBS events: n/a <b>not in passed_optdep</b>'),
                          Paragraph('AEBS warning rate: n/a'),
                          Spacer(width=1 * cm, height=0.2 * cm), ]

            # AEBS warning rate for ARS430
            story += [Spacer(width=1 * cm, height=0.2 * cm),
                      Paragraph(bold('ARS430:')), ]
            if (self.optdep['ARS430aebs_events'] in self.passed_optdep and
                        self.optdep['count_vs_aebs_severity'] in self.passed_optdep):
                aebs_ei_ids = index_fill(self.modules.fill(self.optdep['ARS430aebs_events']))
                header = cIntervalHeader.fromFileName(self.query_files['aebs_events'])
                table = self.batch.get_table_dict(header, aebs_ei_ids,
                                                  sortby=[('measurement', True), ('start [s]', True)])
                #global AEBS_TABLE
                ...
                if 'roadt_dist' in locals() and roadt_dist.total > 0.0:
                    ...
                    story += [Paragraph('AEBS warning rate - overall: <b>%.1f events / 1000 km</b>' % tot_rate),
                              Paragraph(
                                  'AEBS warning rate - 1-False alarm: <b>%.1f events / 5000 km</b>' % false_rate),
                              Paragraph(
                                  'AEBS warning rate - 2-Questionable false alarm: <b>%.1f events / 400 km</b>' % ques_rate),
                              Spacer(width=1 * cm, height=0.2 * cm), ]
                else:
            ...
            story += [Table([
                ...
                [m_plot("view_quantity_vs_systemstate_statsARS430-pie_duration@aebseval.systemstate")],
            ], style=table_style)]
            return

        story = [IndexedParagraph('Overall summary', style='Heading1'),
                 IndexedParagraph('Cumulative results', style='Heading2'),
                 Paragraph(bold('General:')),
        ]
    ...
    def aebs_event_classification(self):
        # AC100 AEBS event classification ------------------------------------------------------------------------
        story = [IndexedParagraph('AEBS event classification by sensors', style='Heading1'),
                 IndexedParagraph('AC100 AEBS event classification', style='Heading2')]
        ...
        story += [Table([
            [m_plot("view_count_vs_phase_stats-bar_FLR20_mergedcount@aebseval.classif"),
             m_plot("view_count_vs_movstate_stats-bar_FLR20_mergedcount@aebseval.classif")],
            [m_plot("view_count_vs_severity_stats-bar_FLR20_mergedcount@aebseval.classif"),
             m_plot("view_count_vs_cause_maingroup_stats-bar_FLR20_mergedcount@aebseval.classif")],
        ], style=table_style)]
        ...
        # ARS430 AEBS event classification -----------------------------------------------------------------------
        story += [IndexedParagraph('ARS430 AEBS event classification', style='Heading2')]

        story += [Table([
            [m_plot("view_count_vs_phase_stats-bar_ARS430_mergedcount@aebseval.classif"),
             m_plot("view_count_vs_movstate_stats-bar_ARS430_mergedcount@aebseval.classif")],
            [m_plot("view_count_vs_severity_stats-bar_ARS430_mergedcount@aebseval.classif"),
             m_plot("view_count_vs_cause_maingroup_stats-bar_ARS430_mergedcount@aebseval.classif")],
        ], style=table_style)]
...
class Flr20Summary(AebsSummary):
  ...
  """ \
    ...
    extra_modules = [
        ('view_driveract_aebsout_ARS430@aebseval',
         Client('DriverAct_AebsOut_Plot', '640x700+0+0', 11, 12, cm)),
        ('view_kinematics_ARS430@aebseval',
         Client('Kinematics_Plot', '640x760+0+0', 11, 13.8, cm)),
    ]
    statuses = ['fillFLR20_AEB@aebs.fill', 'fillContiARS430@aebs.fill', 'fillContiARS430_AEBS@aebs.fill']
    groups = ['FLR20_AEB', 'ContiARS430', 'Conti_AEBS']
