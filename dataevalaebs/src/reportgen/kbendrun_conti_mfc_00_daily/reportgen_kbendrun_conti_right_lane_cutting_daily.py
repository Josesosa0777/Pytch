    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            if self.optdep['lane_cutting_analysis'] in self.passed_optdep:
                ...
                global LANE_CUTTING_ANALYSIS, right_lane_departure
                LANE_CUTTING_ANALYSIS = table
                for id, value in enumerate(LANE_CUTTING_ANALYSIS):
                    if LANE_CUTTING_ANALYSIS[id]['LaneDepartureWarning'][0] == u'RightLaneDepartureWarning':
                        right_lane_departure.append(LANE_CUTTING_ANALYSIS[id])
                story += [Paragraph('Total LaneDepartureWarning Events : %d' % len(right_lane_departure))]
...
class LaneDepartureWarningEvent(EventSummary):
    ...
    def init(self, batch, view_name):
        ...
        self.modules.update([
            ('view_nxtvideoeventgrabbernav_lanes-FLC20@evaltools',
