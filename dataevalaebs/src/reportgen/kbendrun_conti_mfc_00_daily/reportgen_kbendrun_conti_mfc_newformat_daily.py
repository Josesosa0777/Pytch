...
from datalab.tygra import IndexedParagraph, Paragraph, NonEmptyTableWithHeader, \
    italic, bold, grid_table_style, dtc_table
...
dtcMappingFile_path_Radar = os.path.join(os.path.dirname(__file__), "config", "dtc_mapping_radar.txt")

DETECTED_OBJS_TABLE = None
RESET_TABLE = None
DTC_TABLE = None
DTC_TABLE_RADAR = None
dtcMappningDict = {}
dtcMappningDict_Radar = {}
...
class AebsAnalyze(Analyze):
    optdep = dict(
        ...
        dtc_events_radar='analyze_flr25_dtc_active_events-last_entries@flr25eval.faults',
        ...
        Lane_Width_Jump_event="analyze_flc25_Jumps_Event-last_entries@mfc525eval.laneeval",
    ...
    query_files = {
        ...
        'dtc_events_radar': abspath('../../flr25eval/faults/flr25_active_dtc_inittable.sql'),
        ...
        'num_of_used_objects_detected_events': abspath(
            '../../mfc525eval/objecteval/flc25_num_of_detected_objs_inittable.sql'),
        'construction_site_event': abspath('../../mfc525eval/laneeval/flc25_construction_site_inttable.sql'),
        'Lane_Width_Jump_event': abspath('../../mfc525eval/laneeval/flc25_Lane_Width_Jump_inttable.sql'),
    ...
    def fill(self):
        ...
        summaries = [  # LQSummary(self.batch, self.view_name), LJSummary(self.batch, self.view_name),
            ...
            LaneWidthJumpSummary(self.batch, self.view_name)
        ]
        ...
        flr25_dtc_summary = [DTCSummary_Radar(self.batch, self.view_name)]
        ...
        story.extend(self.flr25_dtc_summary(flr25_dtc_summary))
    ...
    def cal_acal_stat(self, table):
        ...
        for val in table:
            ...
            if not yaw_stat_per_meas.has_key(Day):
                yaw_stat_per_meas[Day] = {'Count': [], 'Sum': [], 'Mean': [], 'Std': [], 'Min': [], 'Max': [],
                                          'Violation': [], 'comment': val['comment']}
            ...
            yaw_stat_per_meas[Day]['Std'].append(
                val['Std'] * val['Count'])  # Std * count -------for one meas-----will make list of this cal
        ...
        for day_val, sample in zip(yaw_stat_per_meas.keys(), yaw_stat_per_meas.values()):
            yaw_stat_per_day[day_val] = {'Count': [], 'Sum': [], 'Mean': [], 'Std': [], 'Min': [], 'Max': [],
                                         'Violation': [], 'comment': sample['comment']}
        ...
        yaw_stat = {'Count': total_cnt, 'Mean': mean, 'Std': std, 'Min': min_val, 'Max': max_val,
                    'Violation': violation, 'Diff': diff, 'Comment': yaw_stat_compaign['comment']}
        stat_table = {}
    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            if self.optdep['Lane_Width_Jump_event'] in self.passed_optdep:
                lane_width_ei_ids = index_fill(self.modules.fill(self.optdep['Lane_Width_Jump_event']))
                header = cIntervalHeader.fromFileName(self.query_files['Lane_Width_Jump_event'])
                table = self.batch.get_table_dict(header, lane_width_ei_ids,
                                                  sortby=[('measurement', True), ('start [s]', True)])
                global LANE_WIDTH_JUMP_TABLE
                LANE_WIDTH_JUMP_TABLE = table
                story += [Paragraph('Total Lane Width Events : %d' % len(table))]
            ...
            if (self.optdep['dtc_events'] in self.passed_optdep):
                ...
                story += [Paragraph('Camera DTC Overview Table:')]
                ...
                col_widths = [3 * cm, 3 * cm, 8 * cm]
                story.append(
                    Table([('DTCs', 'No of Events', 'DTC Name')], colWidths=col_widths, style=dtc_table,
                          vAlign='MIDDLE'))

                for dtc_name, dtc_value in dtc_dictionary.iteritems():
                    try:
                        story.append(
                            Table([(dtc_name, '%d' % (dtc_value), dtcMappningDict[dtc_name])], colWidths=col_widths,
                                  style=dtc_table,
                                  vAlign='MIDDLE'))
                    except:
                        story.append(
                            Table([(dtc_name, '%d' % (dtc_value), 'DTC_Mapping_ID_Missing')], colWidths=col_widths,
                                  style=dtc_table,
                                  vAlign='MIDDLE'))

            story.append(Spacer(width=1 * cm, height=0.5 * cm))

            # DTC event table Radar
            if (self.optdep['dtc_events_radar'] in self.passed_optdep):
                dtc_ei_ids = index_fill(self.modules.fill(self.optdep['dtc_events_radar']))
                header1 = cIntervalHeader.fromFileName(self.query_files['dtc_events_radar'])
                dtc_table_internal = self.batch.get_table_dict(header1, dtc_ei_ids,
                                                               sortby=[('measurement', True), ('start [s]', True)])
                global DTC_TABLE_RADAR, dtcMappningDict_Radar
                DTC_TABLE_RADAR = dtc_table_internal

                story += [Paragraph('Radar DTC Overview Table:')]

                story.append(Spacer(width=1 * cm, height=0.5 * cm))

                dtc_dictionary_r= {}

                dtc_list_in_hex_r = [hex(int(dtc_list['DTC in DEC'])) for dtc_list in dtc_table_internal]

                previoud_dtc_id = ""
                refined_dtc_list = []

                for key in dtc_list_in_hex_r:
                    if key != previoud_dtc_id:
                        if len(key) < 8:
                            key = (key).replace('x', 'x0')
                        refined_dtc_list.append(key)
                ...
                dtcMappingFile = open(dtcMappingFile_path_Radar, 'r')

                for line in dtcMappingFile:
                    dtc_id, dtc_name = line.strip().split('=')
                    dtcMappningDict_Radar[dtc_id.strip().lower()] = dtc_name.strip()

                dtc_dictionary_r = {key: refined_dtc_list.count(key) for key in refined_dtc_list}
                ...
                for dtc_name, dtc_value in dtc_dictionary_r.iteritems():
                    try:
                        story.append(
                            Table([(dtc_name, '%d' % (dtc_value), dtcMappningDict_Radar[dtc_name])], colWidths=col_widths,
                                  style=dtc_table,
                                  vAlign='MIDDLE'))
                    except:
                        story.append(
            ...
            story.append(Spacer(width=1 * cm, height=0.5 * cm))
                ...
                #
                # story.append(
                #     Table([('DTCs', 'No of Events')], colWidths=4 * cm, style=grid_table_style, vAlign='MIDDLE'))
                #
                # for dtc_name, dtc_value in dtc_dictionary_r.iteritems():
                #     story.append(Table([(dtc_name, '%d' % (dtc_value))], colWidths=4 * cm, style=grid_table_style,
                #                        vAlign='MIDDLE'))
                #
                # dtcMappingFile = open(dtcMappingFile_path_Radar, 'r')
                #
    ...
    def flr25_dtc_summary(self, summaries, module_name=None):
        story = [
            IndexedParagraph('FLR25 DTC Summary', style='Heading1'),
            NextPageTemplate('LandscapeTable'),
        ]
        if module_name is not None:
            story.insert(1, self.module_plot(module_name))
...
class LaneWidthJumpEvent(EventSummary):

    def init(self, batch, view_name):
        data = LANE_WIDTH_JUMP_TABLE
        ...
        for row in data:
            self.setdefault(row['fullmeas'], []).append(dict(
                ...
                duration=row['duration [s]'],
                speed=row['frontAxleSpeed [km/h]'],
                event=vector2scalar(row['LaneWidthJump']),
            ))
...
class ODSummary(LaneChangeSummary):
    ...
    explanation = Object ID %s - Lane changed from: %s Lane changed to: %s<br/>
...
class OJSummary(ObjJumpSummary):
    ...
    #explanation = Object ID %s - Delta distance X: %s dx distance: %s<br/>
...
class LQSummary(LaneQualitySummary):
    ...
    explanation = Lane Quality: %s, Event Duration: %s \
                  % ('%(event)s', '%(duration)s')
    EventSummary.stat_im = Image(os.path.join(PIC_DIR, 'flr25_stationary_legend.png'),
                                 width=0.5 * cm, height=0.5 * cm)
    EventSummary.mov_im = Image(os.path.join(PIC_DIR, 'flr25_moving_legend.png'),
...
class LQWSummary(LaneQualityWorstSummary):
    ...
    #explanation = Lane Worst Quality Drop: %s, Event Duration: %s\
                  % ('%(event)s', '%(duration)s')
    EventSummary.stat_im = Image(os.path.join(PIC_DIR, 'flr25_stationary_legend.png'),
                                 width=0.5 * cm, height=0.5 * cm)
    EventSummary.mov_im = Image(os.path.join(PIC_DIR, 'flr25_moving_legend.png'),
...
class LDSummary(LdwsSummary):
    ...
    explanation = LDWS event: %s, Event Duration: %s\
    % ('%(event)s', '%(duration)s')
    EventSummary.stat_im = Image(os.path.join(PIC_DIR, 'flr25_stationary_legend.png'),
                                 width=0.5 * cm, height=0.5 * cm)
    EventSummary.mov_im = Image(os.path.join(PIC_DIR, 'flr25_moving_legend.png'),
...
class CompareC0FLCvsMFC525Summary(CompareC0FLCvsMFC525):
    ...
    explanation = FLCvsMFC525 Compare C0 event: %s,  Event Duration: %s \
                  % ('%(event)s', '%(duration)s')
    EventSummary.stat_im = Image(os.path.join(PIC_DIR, 'flr25_stationary_legend.png'),
                                 width=0.5 * cm, height=0.5 * cm)
    EventSummary.mov_im = Image(os.path.join(PIC_DIR, 'flr25_moving_legend.png'),
    ...
    groups = ['FLC25_AOA_AEBS']
"""
class LaneWidthJumpSummary(LaneWidthJumpEvent):
    title = "Lande Width Jump"
    explanation = """Lane Width Jump Event: %s,  Event Duration: %s""" % ('%(event)s', '%(duration)s')
    ...
    extra_modules = [
        ('view_flc25_Jump_event@mfc525eval.laneeval',
         Client('Lande Width Jump Event', '640x700+0+0', 11, 12, cm)),
        ('view_flc25_Jump_event_v2@mfc525eval.laneeval',
         Client('Lande Width Jump Event', '640x700+0+0', 11, 12, cm)),
    ]
    statuses = ['fillFLC25_AOA_AEBS@aebs.fill']
    groups = ['FLC25_AOA_AEBS']
...
class ConstructionSiteSummary(ConstructionSiteEvent):
    ...
    extra_modules = [
        ...
        ('view_flc25_constructionSite_version2@mfc525eval.laneeval',
         Client('Construction Site Event', '640x700+0+0', 11, 12, cm)),
    ]
    statuses = ['fillFLC25_AOA_AEBS@aebs.fill']
    groups = ['FLC25_AOA_AEBS']
...
class Flr25Summary(AebsSummary):
    ...
    #explanation = AEBS - event duration: %s sec, cause: %s, rating: %s<br/>
    ...
    groups = ['FLC25_AOA_AEBS']
"""
class DTCSummary_Radar(Summary):
    def init(self, batch, view_name):
        data = DTC_TABLE_RADAR
    ...
    def get_data(self, link_pattern, link_heading):
        ...
        for meas, resets in self.iteritems():
            ...
            for reset in resets:
                ...
                if reset['dtc_id_in_hex'] in dtcMappningDict_Radar:
                    row = [basename, reset['dtc_id'], reset['dtc_id_in_hex'], reset['dtc_counter'],
                           reset['dtc_timestamp'], dtcMappningDict_Radar[reset['dtc_id_in_hex']],
                           reset['dtc_comment']]
...
class ACAlYawStat(Summary):
    ...
    def get_data(self, link_pattern, link_heading):
        ...
        for meas, resets in self.iteritems():
            basename = os.path.basename(meas)
            for reset in resets:
                row = [basename, reset['count'], reset['mean'], reset['std'], reset['min'], reset['max'], reset['diff'],
                       reset['Violation'],
                       reset['comment']]
                data.append(row)
        return data
