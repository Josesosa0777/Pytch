...
from datalab.tygra import (
    IndexedParagraph,
    Paragraph,
    NonEmptyTableWithHeader,
    italic,
    bold,
    grid_table_style,
)
...
class ACCAnalyze(Analyze):
    dep = {
        "acc_events": "analyze_events-last_entries@accpedstop",
    }
    optdep = dict(
    ...
    query_files = {
        "acc_events": abspath("../../accpedstop/events_inttable.sql"),
        "vehicle_in_speeed_range": abspath(
            "../../accpedresim/accped_speed_range_inttable.sql"
        ),
    ...
    def fill(self):
        ...
        ACC_TABLE.append(
            self.batch.get_table_dict(
                header, acc_dirk, sortby=[("measurement", True), ("start [s]", True)]
            )
        )
    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            story += [
                Paragraph("Analyzed time period: %s" % from_to),
                Spacer(width=1 * cm, height=0.2 * cm),
            ]

            # driven distance and duration
            if (
                self.optdep["dur_vs_roadtype"] in self.passed_optdep
                and self.optdep["dist_vs_roadtype"] in self.passed_optdep
            ):
                roadt_dur = index_fill(
                    self.modules.fill(self.optdep["dur_vs_roadtype"])
                )
                roadt_dist = index_fill(
                    self.modules.fill(self.optdep["dist_vs_roadtype"])
                )

                # distance
                if roadt_dist.total > 0.0:
                    calc_dist_perc = lambda d: int(
                    ...
                    story += [
                        Paragraph(
                            "Total mileage: %s" % bold("%.1f km" % roadt_dist.total)
                        )
                    ]
                # duration
                if roadt_dur.total > 0.25:
                    ...
                    story += [
                        Paragraph(
                            "Road type statistics, Total duration: %s"
                            % bold("%.2f hours" % roadt_dur.total)
                        )
                    ]
            else:
                self.logger.warning("Road type statistics not available")
                story += [
                    Paragraph("Total duration: n/a"),
                    Paragraph("Total mileage: n/a"),
                ]
            # engine running
            if self.optdep["dur_vs_engine_onoff"] in self.passed_optdep:
                engine_dur = index_fill(
                    self.modules.fill(self.optdep["dur_vs_engine_onoff"])
                )
                if "roadt_dur" in locals():
                    # plau check for durations of different sources
                    if (
                        roadt_dur.total > 0.25
                        and abs(1.0 - engine_dur.total / roadt_dur.total) > 0.02
                    ):  # 2% tolerance
                        self.logger.error(
                            "Different duration results: %.2f h (engine state) "
                            "vs. %.2f h (road type)"
                            % (engine_dur.total, roadt_dur.total)
                        )
                # duration
                if engine_dur.total > 0.0:
                    story += [
                        Paragraph(
                            ...
                            % (
                                engine_dur.total,
                                100.0 * engine_dur["yes"] / engine_dur.total,
                    ...
                    story += [
                        Paragraph(
                            "Engine state statistics, Total duration: %.2f hours"
                            % engine_dur.total
                        )
                    ]
            else:
            ...
            if self.optdep["dur_vs_daytime"] in self.passed_optdep:
                ...
                if "roadt_dur" in locals():
                    ...
                    if (
                        roadt_dur.total > 0.25
                        and abs(1.0 - daytime_dur.total / roadt_dur.total) > 0.02
                    ):  # 2% tolerance
                        self.logger.error(
                            "Different duration results: %.2f h (daytime) "
                            "vs. %.2f h (road type)"
                            % (daytime_dur.total, roadt_dur.total)
                        )
                # duration
                if daytime_dur.total > 0.0:
                    ...
                    story += [
                        Paragraph(
                            "Daytime statistics, Total duration: %.2f hours"
                            % daytime_dur.total
                        )
                    ]
            else:
            ...
            if self.optdep["vehicle_in_speeed_range"] in self.passed_optdep:
                percentage_vehicle_in_accped_speed_range = 0
                vehicle_in_speeed_range = index_fill(
                    self.modules.fill(self.optdep["vehicle_in_speeed_range"])
                )
                vehicle_in_speeed_range_header = cIntervalHeader.fromFileName(
                    self.query_files["vehicle_in_speeed_range"]
                )
                vehicle_in_speed_range_data = self.batch.get_table_dict(
                    vehicle_in_speeed_range_header,
                    vehicle_in_speeed_range,
                    sortby=[("measurement", True), ("start [s]", True)],
                )
                try:
                    percentage_vehicle_in_accped_speed_range = (
                        vehicle_in_speed_range_data[0]["vehicle_in_accped_speed_range"]
                        * 100
                    )
                except:
                    pass
                story += [
                    Paragraph(
                        "Vehicle in ACC PED STOP speed range: %s"
                        % bold("%d%%" % percentage_vehicle_in_accped_speed_range)
                    )
                ]
            ...
            story += [
                Paragraph(
                    italic(
                        'Remark: Percentage values with "ca." are '
    ...
    def aebs_event_classification(self):
        ...
        m_plot = lambda m: self.module_plot(
            m, windgeom="500x300+0+0", width=60.0, height=60.0, unit=1.0, kind="%"
        )
        table_style = [
            ("ALIGN", (0, 0), (-1, -1), "RIGHT"),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ...
        story += [
            Table(
                [
                    [
                        m_plot(
                            "view_count_vs_phase_stats-bar_mergedcount@aebseval.classif"
                        ),
                        m_plot(
                            "view_count_vs_movstate_stats-bar_mergedcount@aebseval.classif"
                        ),
                    ],
                    [
                        m_plot(
                            "view_count_vs_severity_stats-bar_mergedcount@aebseval.classif"
                        ),
                        m_plot(
                            "view_count_vs_cause_maingroup_stats-bar_mergedcount@aebseval.classif"
                        ),
                    ],
                ],
                style=table_style,
    ...
    def summaries_acc(self, summaries, module_name=None):
        ...
        for summary in summaries:
            ...
            story.append(
                summary.get_table(link_pattern=self.EVENT_LINK, link_heading="Heading2")
            ),
            story.append(PageBreak())
        return story

    def events(self, summary):
        statuses = ["fillFLR25_ACC_PED_STOP@aebs.fill"]
        groups = ["FLR25_ACC_PED_STOP"]
        ...
        for meas, warnings in summary.iteritems():
            ...
            manager.strong_time_check = (
                False  # TODO: make default behavior with warning
            )
            manager.set_measurement(meas)
            manager.build(
                summary.modules,
                status_names=statuses,
                visible_group_names=groups,
...
class ACCSummary(EventSummary):
    def init(self, batch, view_name):
        ...
        for row in valid_data:
            for i in row:
                self.setdefault(i["fullmeas"], []).append(
                    dict(
                        start=i["start [s]"],
...
class Summary(ACCSummary):
    ...
    extra_modules = [
        ...
        (
            "view_acc_ped_stop_evaluation@accpedstop",
            Client("ACC PED Stop plots", "640x700+0+0", 11, 12, cm),
        ),
    ]
