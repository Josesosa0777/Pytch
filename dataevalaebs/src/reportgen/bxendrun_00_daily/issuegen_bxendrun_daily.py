...
class Analyze(iAnalyze):
  optdep = dict(
    aebs_events='analyze_bendix_warning_cmt-last_entries@aebseval',
    umo_events='analyze_bendix_warning_umo-last_entries@aebseval',
    soa_events='analyze_bendix_warning_soa-last_entries@aebseval',
  ...
  def fill(self):
    ...
    story += [IndexedParagraph('FLR21 AEBS warnings', 'Heading2')]
    #-------------------------------------------------------------
    if self.optdep['aebs_events'] in self.passed_optdep:
      ...
      self.logger.warning('FLR21 AEBS warnings not available')
      story += [Paragraph('Information not available'),
                Spacer(width=1*cm, height=0.2*cm),]

    story += [IndexedParagraph('FLR21 UMO warnings', 'Heading2')]
    #-------------------------------------------------------------
    if self.optdep['umo_events'] in self.passed_optdep:
      header = cIntervalHeader.fromFileName(abspath('../../aebseval/events_inttable.sql'))
      ids = index_fill(self.modules.fill(self.optdep['umo_events']))
      table = self.batch.get_table(header, ids, sortby=[('measurement', True), ('start [s]', True)])
      table_chunk = [row[1:-1] for row in table]
      story += [NonEmptyTableWithHeader(table_chunk),
                Spacer(width=1*cm, height=0.2*cm),]
      ...
      self.logger.warning('FLR21 UMO warnings not available')
      story += [Paragraph('Information not available'),
                Spacer(width=1*cm, height=0.2*cm),]

    story += [IndexedParagraph('FLR21 SOA warnings', 'Heading2')]
    #-------------------------------------------------------------
    if self.optdep['soa_events'] in self.passed_optdep:
      header = cIntervalHeader.fromFileName(abspath('../../aebseval/events_inttable.sql'))
      ids = index_fill(self.modules.fill(self.optdep['soa_events']))
      table = self.batch.get_table(header, ids, sortby=[('measurement', True), ('start [s]', True)])
      table_chunk = [row[1:-1] for row in table]
      story += [NonEmptyTableWithHeader(table_chunk),
                Spacer(width=1*cm, height=0.2*cm),]
      ...
      self.logger.warning('FLR21 SOA warnings not available')
      story += [Paragraph('Information not available'),
                Spacer(width=1*cm, height=0.2*cm),]
