...
class AebsAnalyze(Analyze):
    ...
    def summaries(self, summaries, module_name=None):
        ...
        story += [Paragraph(italic('1. Positive event start time difference indicates resimulation event occurs after measurement event.'),fontsize=8)]
    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            try:
                if (self.optdep['aebs_detect_event'] in self.passed_optdep):
                    aebs_ei_ids = index_fill(self.modules.fill(self.optdep['aebs_detect_event']))
                    header = cIntervalHeader.fromFileName(self.query_files['aebs_detect_event'])
            ...
            # AEBS Resim Table
            # if (self.optdep['aebs_detect_event'] in self.passed_optdep):
            #     resim_ei_ids = index_fill(self.modules.fill(self.optdep['aebs_detect_event']))
            #     header1 = cIntervalHeader.fromFileName(self.query_files['aebs_detect_event'])
            #     resim_table_internal = self.batch.get_table_dict(header1, resim_ei_ids,
            #                                                    sortby=[('measurement', True), ('start [s]', True)])
            #     global AEBS_RESIM_TABLE
            #     AEBS_RESIM_TABLE = resim_table_internal
            #
            ...
            tot_resim = 0
            tot_aebs = 0
            if (self.optdep['aebs_detect_event'] in self.passed_optdep and
            ...
            #     Resimulation cumulative result
                    # AEBS endurance details
                story += [Paragraph('Total number of AEBS resim events: %d' % tot_resim)]
                if total_mileage:
                    if 'roadt_dist' in locals() and total_mileage > 0.0:
    ...
    def events(self, summary):
        statuses = ['fillFLR25_AEBS_RESIM@aebs.fill','fillFLR25_AEB@aebs.fill']
        statuses.extend(summary.statuses)
        groups = ['FLR25_AEB_RESIM', 'FLR25_AEB']
    ...
    def init(self, batch, view_name):
        ...
        for row in data:
            self.setdefault(row['fullmeas'],[]).append(dict(
                ...
                resim_rating_useCaseMode=vector2scalar(row['resim useCaseMode warning rating scale']),
...
class ResimParameters(Summary):
    ...
    def get_data(self, link_pattern, link_heading):
        ...
        conf.read(CONFIG_PATH.split('reportgen')[0] + r"evalbot\botconfigs\config_templates\EnduranceRun_Conti_AEBS_delta_resim.cfg")
        try:
        ...
        try:
            ...
            measurement_signal_values = json.load(open(measurement_signals_json))

            if not bool(measurement_signal_values):
                logger.error("Measurement data is not available due to missing signal.")

            json_file = open(glob.glob((os.path.join(jsonPath, '*.json')))[0])
...
class Flr25Summary(AebsSummary):
    ...
    explanation = """
	AEBS event: %s & AEBS resim event: %s - event duration: %s sec, Event source: %s, Rating: %s<br/>
	resimEventRating: %s, useCaseModeRating: %s. Event is triggered because AEBS state was active: warning (%d), partial (%d)
	or emergency (%d) braking.
	""" \
                  % ('%(phase)s','%(event)s', '%(duration).2f', '%(event_source)s','%(meas_rating)s','%(resim_rating)s','%(resim_rating_useCaseMode)s',
    ...
    extra_modules = [
         ('view_driveract_aebsout@aebseval.resim',
         Client('DriverAct_AebsOut_Plot', '640x700+0+0', 11, 12, cm)),
        # ('view_aebs_resim@aebseval.resim',
        #  Client('DriverAct_Ego_Plot', '640x700+0+0', 11, 12, cm)),
        ('view_kinematics_FLR25@aebseval.resim',
