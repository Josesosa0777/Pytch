...
class AebsAnalyze(Analyze):
    optdep = dict(
        yaca_events='analyze_yaca_events-last_entries@yacainterface',
    ...
    query_files = {
        'yaca_events': abspath('../../yacainterface/yaca_events_inttable.sql'),
    ...
    def overall_summary(self):
        def one_summary(story, start_date, end_date, index_fill):
            ...
            # AEBS warning rate
            if (self.optdep['yaca_events'] in self.passed_optdep and
                    self.optdep['count_vs_aebs_severity'] in self.passed_optdep):
                aebs_ei_ids = index_fill(self.modules.fill(self.optdep['yaca_events']))
                header = cIntervalHeader.fromFileName(self.query_files['yaca_events'])
    ...
    def events(self, summary):
        statuses = ['fillFLR25_AEBS_RESIM@aebs.fill']
        statuses.extend(summary.statuses)
        groups = ['FLR25_AEB_RESIM']
