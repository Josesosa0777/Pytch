...
class AebsAnalyze(Analyze):
	optdep = dict(
		...
		fcw_events='analyze_fcwevents_merged-last_entries@fcweval',
		...
		count_vs_aebs_severity='view_count_vs_severity_stats_flr25-print_mergedcount@fcweval.classif',
	...
	query_files = {
		...
		'fcw_events': abspath('../../fcweval/events_inttable.sql'),
	...
	def overall_summary(self):
		def one_summary(story, start_date, end_date, index_fill):
			...
			story += [Table([
				[m_plot("view_quantity_vs_systemstate_stats_flr25-pie_duration@fcweval.systemstate")],
				[m_plot("view_quantity_vs_systemstate_stats_flr25_mileage-pie_mileage@fcweval.systemstate")],
			], style=table_style)]
	...
	def aebs_event_classification(self, dtc_summary):
		...
		story = [IndexedParagraph('Event overview', style='Heading1'), NextPageTemplate('LandscapeTable'), ]
		...
		story += [Table([
			[m_plot("view_count_vs_phase_stats_fcw-bar_mergedcount@fcweval.classif"),
			 m_plot("view_count_vs_movstate_stats_flr25-bar_mergedcount@fcweval.classif")],
			[m_plot("view_count_vs_severity_stats_flr25-bar_mergedcount@fcweval.classif"),
			 m_plot("view_count_vs_cause_maingroup_stats_flr25-bar_mergedcount@fcweval.classif")],
		], style=table_style)]
		story += [PageBreak()]
	...
	def get_data(self, link_pattern, link_heading):
		...
		for meas, dtcs in self.iteritems():
			...
			for dtc in dtcs:
				...
				row = [basename, hex(int(dtc['dtc_id'])), dtc['dtc_counter'], dtc['dtc_timestamp'], dtc_name,
					   dem_events, dtc['dtc_comment']]
				data.append(row)
		return data
