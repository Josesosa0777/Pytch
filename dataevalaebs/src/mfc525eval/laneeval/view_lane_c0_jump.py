...
class View(iView):
    ...
    def view(self, group, commonTime):
        ...
        ax = pn.addAxis(ylabel="lane_c0_jump")
        if 'left_lane_marking_c0' in group:
            time00, left_lane_value00, unit00 = group.get_signal_with_unit("left_lane_marking_c0")
            pn.addSignal2Axis(ax, "left_lane_marking_c0", time00, left_lane_value00, unit=unit00)

        # ax = pn.addAxis(ylabel="Right_lane_c0_jump")
        if 'right_lane_marking_c0' in group:
            time00, right_lane_value00, unit00 = group.get_signal_with_unit("right_lane_marking_c0")
            pn.addSignal2Axis(ax, "right_lane_marking_c0", time00, right_lane_value00, unit=unit00)

        ax = pn.addAxis(ylabel="sample diff.", ylim=(-0.8, 0.8))
        if 'left_lane_marking_c0' in group:
            time00, left_lane_value00, unit00 = group.get_signal_with_unit("left_lane_marking_c0", **rescale_kwargs)
            left_c0_diff = np.append(np.diff(left_lane_value00), 0)
            pn.addSignal2Axis(ax, "left_c0_jump", commonTime, left_c0_diff, unit=unit00)

        if 'right_lane_marking_c0' in group:
            time00, right_lane_value00, unit00 = group.get_signal_with_unit("right_lane_marking_c0", **rescale_kwargs)
            right_c0_diff = np.append(np.diff(right_lane_value00), 0)
            pn.addSignal2Axis(ax, "right_c0_jump", commonTime, right_c0_diff, unit=unit00)
        ...
        ax = pn.addAxis(ylabel="Exit&Stop")
