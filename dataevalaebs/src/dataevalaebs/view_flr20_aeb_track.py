...
class MyView(iView):
  ...
  def view(self, track):
    pn = datavis.cPlotNavigator(title='FLR20 AEB track')
    t = track.time
    # id
    ax = pn.addAxis(ylabel='id')
    pn.addSignal2Axis(ax, 'radar id', t, track.id)
    ...
    # ego speed (hack)
    pn.addSignal2Axis(ax, 'ego speed', t, track._tracks.itervalues().next()._ego.vx, unit='m/s')
    # ttc
    ax = pn.addAxis(ylabel='ttc')
    pn.addSignal2Axis(ax, 'ttc', t, track.ttc, unit='s')
    # conf
