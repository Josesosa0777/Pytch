...
SELECT DISTINCT asso_late.ei_id, meas_tb.basename,
                MAX(asso_late.start_time, aeb_tb.start_time) AS common_start_time,
                MIN(asso_late.end_time, aeb_tb.end_time) AS common_end_time,
                MIN(asso_late.end_time, aeb_tb.end_time) - MAX(asso_late.start_time, aeb_tb.start_time) AS duration,
                egospeed_tb.val, ttc_tb.val, asso_late.label,
...
JOIN (
  ...
  ) aeb_tb ON     asso_late.me_id = aeb_tb.me_id
                    AND asso_late.label = aeb_tb.label
                    AND MAX(asso_late.start_time, aeb_tb.start_time) <= MIN(asso_late.end_time, aeb_tb.end_time)
JOIN (
  SELECT ei.id ei_id
  FROM entryintervals ei
  JOIN interval2label il ON ei.id = il.entry_intervalid
  ...
  WHERE     lg.name = "asso problem"
        AND la.name = "late"
  ) latency_tb ON asso_late.ei_id = latency_tb.ei_id
JOIN (
...
--      AND q.value > 6
        AND q.value < 60
  ) dx_tb ON asso_late.ei_id = dx_tb.ei_id
JOIN (
  SELECT ei.id ei_id, q.value as val
  FROM entryintervals ei
  JOIN quantities q      ON ei.id = q.entry_intervalid
  ...
  WHERE     qng.name = "target"
        AND qn.name = "ttc min"
        AND q.value < 50
  ) ttc_tb ON asso_late.ei_id = ttc_tb.ei_id
JOIN (
  ...
  ) movst_tb ON asso_late.ei_id = movst_tb.ei_id
JOIN (
  SELECT ei.id ei_id, q.value AS val
  FROM entryintervals ei
  JOIN quantities q      ON ei.id = q.entry_intervalid
  ...
  WHERE     qng.name = "ego vehicle"
        AND qn.name = "speed"
        AND q.value > 20
  ) egospeed_tb ON asso_late.ei_id = egospeed_tb.ei_id
JOIN (
  SELECT meas.id me_id, meas.basename
  FROM measurements meas
  ) meas_tb ON asso_late.me_id = meas_tb.me_id
