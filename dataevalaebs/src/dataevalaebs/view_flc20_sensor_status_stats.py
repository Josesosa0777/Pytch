...
camera_status_query_template = """
SELECT la.name status, strftime('%s', me.start, '-4 hour') d, SUM(ei.end_time - ei.start_time) / 3600.
...
JOIN results re        ON en.resultid = re.id
JOIN measurements me   ON en.measurementid = me.id
WHERE     lg.name = "camera status"
      AND mo.class = "dataevalaebs.search_flc20_sensor_status.Search"
      AND re.name = "passed"
GROUP BY status %s
;
"""
...
JOIN modules mo ON en.moduleid = mo.id
WHERE mo.class = "dataevalaebs.search_flc20_sensor_status.Search" --avoid later updates
ORDER BY days ASC;
"""

titlestr = """
FLC20 sensor status in endurance drive (between %s and %s)
...
class View(iView):
  def init(self, strftime, groupby, bar_chart):
    ...
    self.colors = OrderedDict(
      ( # color order determines how bars are stacked on top of each other (bottom-up)
  ...
  def check(self):
    ...
    camera_status_query = camera_status_query_template \
                            %(self.strftime, table_name, self.groupby)
    res = self.batch.query(camera_status_query)
  ...
  def fill(self, res, meas_days):
    ...
    title = titlestr %(first_day, last_day, num_days, driven_time)
