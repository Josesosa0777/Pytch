...
import interface
import numpy as np
from evaltools.view_videonav import SignalGroups, getVideoName, multimedia_signal

VIDEO_FILES_CATEGORY = ['MFC5xxDevice', 'Axiscamera1', 'Axiscamera2', 'Axiscamera3', 'Axiscamera4']
...
init_params = {
    ...
    "FLC25_PAEBS_EGO_PATH_PREDICTION": dict(sensors=dict(flc25_ego_pathpred=True)),
...
class cView(interface.iView):
    def init(self, sensors):
        ...
        if self.sensors.get("flc25_ego_pathpred"):
            optdep.append('calc_lanes_flc25_paebs_ego_path_prediction@aebs.fill')
    ...
    def check(self):
        ...
        self.logger.info("Selected video file: %s" % avi_filename)

        if 'MFC5xxDevice' in avi_filename:
            self.selected_video_file = \
                re.findall('MFC5xxDevice_[0-9]+\[[0-9]+\]', os.path.splitext(os.path.basename(avi_filename))[0])[0]
            for id, value in enumerate(multimedia_signal):
                if self.selected_video_file in multimedia_signal[id].keys():
                    multimedia_sgs = [multimedia_signal[id]]
                    self.logger.info('selected multimedia signal is: %s ' % multimedia_sgs)
        elif 'Axiscamera' in avi_filename:
            self.selected_video_file = \
                re.findall('Axiscamera[0-9]_[0-9]+', os.path.splitext(os.path.basename(avi_filename))[0])[0]
            for id, value in enumerate(multimedia_signal):
                if self.selected_video_file in multimedia_signal[id].keys():
                    multimedia_sgs = [multimedia_signal[id]]
                    self.logger.info('selected multimedia signal is: %s' % multimedia_sgs)
        ...
        vidtime_group = self.source.selectLazySignalGroup(multimedia_sgs,
                                                          ...
                                                          TimeMonGapIdx=5)
        if not vidtime_group:
            multimedia_sgs = SignalGroups
            vidtime_group = self.source.selectLazySignalGroup(multimedia_sgs,
                                                              StrictTime=interface.StrictlyGrowingTimeCheck,
                                                              TimeMonGapIdx=5)
        if 'VidTime' not in vidtime_group:
            if self.selected_video_file not in vidtime_group:
    ...
    def view(self, avi_filename, vidtime_group, ldw_group):
        group = None
        if 'VidTime' not in vidtime_group:
            # vidtime_group.get_signal_with_unit('MFC5xxDevice_0800200016[000]')
            TimeVidTime, VidTime = vidtime_group.get_signal(self.selected_video_file)
        elif 'VidTime' in vidtime_group:
        ...
        if 'SLR25_RFB' in self.view_angles or any(
                [bool(True) if 'SLR25_RFB' in modules else bool(False) for modules in
                 list(self.modules._selects)]):
            group = 'SLR25_RFB'
        elif 'SLR25_Front' in self.view_angles or any(
                [bool(True) if 'SLR25_Front' in modules else bool(False) for modules in
                 list(self.modules._selects)]):
            group = 'SLR25_Front'

        objNxtVideoNavigator = datavis.cNxtVideoNavigator(avi_filename, TimeVidTime, VidTime, self.vidcalibs, group)
