...
multimedia_signal = [
    {"Axiscamera1_0206300000": ("Axiscamera1_0206300000", "Axiscamera1_0206300000")},
    {"Axiscamera2_0206300256": ("Axiscamera2_0206300256", "Axiscamera2_0206300256")},
    {"Axiscamera3_0206300512": ("Axiscamera3_0206300512", "Axiscamera3_0206300512")},
    {"Axiscamera4_0206300768": ("Axiscamera4_0206300768", "Axiscamera4_0206300768")},
    {"MFC5xxDevice_0800200016[000]": ("MFC5xxDevice_0800200016_000", "MFC5xxDevice_0800200016[000]")},
    {"MFC5xxDevice_0800200016[001]": ("MFC5xxDevice_0800200016_001", "MFC5xxDevice_0800200016[001]")},
    # MF4, Mat signals
    {"MFC5xxDevice_0800200016[000]": ("MFC5xxDevice_0800200016[000]", "MFC5xxDevice_0800200016[000]")},
    {"MFC5xxDevice_0800200016[001]": ("MFC5xxDevice_0800200016[001]", "MFC5xxDevice_0800200016[001]")},
]
...
SignalGroups = [
    ...
    {
        "VidTime": (
            "MFC5xx Device.FCU.VehDynSync", "MFC5xx_Device_FCU_VehDynSync_Longitudinal_Velocity")
    },
    {
        "VidTime": ("MFC5xx Device.VDY.VehDyn", "MFC5xx_Device_VDY_VehDyn_Longitudinal_Velocity"),
    },
    {"VidTime": ("VehDyn", "SRR520_RightFrontBackwards_AlgoVehCycle_VehDyn_Longitudinal_Velocity")},
    {"VidTime": ("EBC2_0B", "EBC2_FrontAxleSpeed_0B")},
    {"VidTime": ("EBC2_0B", "EBC2_FrontAxleSpeed_0B_s0B")},
    {"VidTime": ("ACC1_2A", "SpeedOfForwardVehicle")},
    {"VidTime": ("VehDyn", "MFC5xx_Device_VDY_VehDyn_Longitudinal_Velocity")},
    {"VidTime": ("MFC5xx Device.VDY.VehDyn", "MFC5xx Device.VDY.VehDyn.Longitudinal.Velocity")},
    ...
    {"Time": ("EBC2_0B", "EBC2_FrontAxleSpeed_0B")},
...
class cView(interface.iView):
    ...
    def check(self):
        ...
        try:
            pyglet.media.load(avi_filename)  # check if loading will be successful
        except:
            raise AssertionError("Invalid/unknown video file: %s" % avi_filename)
        # multimedia signal
...
def getVideoNameInSameDir(MeasFile):
    ...
    for d in deltaSecs:
        ...
        if 'SLR25_RFB' in interface.Manager.groups or any(
                [bool(True) if 'SLR25_RFB' in modules else bool(False) for modules in
                 list(interface.Manager.modules._selects)]):
            BaseBase = os.path.basename(Base) + '_m2_Axiscamera2'
        elif 'SLR25_Front' in interface.Manager.groups or any(
                [bool(True) if 'SLR25_Front' in modules else bool(False) for modules in
                 list(interface.Manager.modules._selects)]):
            BaseBase = os.path.basename(Base) + '_m1_Axiscamera1'
        elif [name for name in files if interface.Manager.source.BaseName.replace('.h5',
                                                                                  '_Axiscamera4') in name] and 'FLC20' in interface.Manager.sync.module or 'FLR25_ROAD_ESTIMATION' in interface.Manager.sync.module:
            BaseBase = os.path.basename(Base) + '_Axiscamera4'
        elif [name for name in files if interface.Manager.source.BaseName.replace('.h5',
                                                                                  '_Axiscamera3') in name] and 'FLC25_ABD_LANE' in interface.Manager.sync.module or 'FLC25_CAN' in interface.Manager.sync.module \
                and 'FLC25_PAEBS_DEBUG' not in interface.Manager.groups:
            BaseBase = os.path.basename(Base) + '_Axiscamera3'
        else:
        ...
        for Ext in supp_exts:
            for f in files:
                if f.startswith(BaseBase) and f.endswith(Ext):  # ca. "glob.glob('%s*%s' % (Base, Ext))" but much faster
                    ...
                    videoSelectionList.append(Name)
            if any('MFC5xxDevice' in sub for sub in videoSelectionList):
                for video_name in videoSelectionList:
                    if 'MFC5xxDevice' in video_name or 'Axiscamera' not in video_name:
                        return video_name
            elif len(videoSelectionList) == 1:
                logger = logging.getLogger()
                logger.warning('Alternative video file selected: %s' % videoSelectionList[0])
                return videoSelectionList[0]
        # else:
...
def getManuallyCorrectedMultimedia(MeasBaseName, VidTime, Logger=None):
    ...
    if MeasBaseName in offsets:
        if Logger is not None: Logger.warning("Multimedia signal manually adjusted")
