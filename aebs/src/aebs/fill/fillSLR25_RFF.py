...
logger = logging.getLogger('fillSLR25_RFF')

INVALID_ID = 255


class cFill(iObjectFill):
		dep = 'fill_slr25_rff_tracks', 'calc_egomotion'

		def check(self):
				modules = self.get_modules()
				tracks, signals = modules.fill("fill_slr25_rff_tracks")
		...
		def fill(self, tracks,signals, egomotion):
				import time
				start = time.time()
				logger.info("SLR25 RFF object retrieval is started, Please wait...")
				objects = []
				for id, track in tracks.iteritems():
						...
						o["label"] = np.array(
										["SLR25_RFF_%d_%s" % (idx, obj_type) for idx, obj_type in zip(o["id"], object_type.data)])
						...
						o["type"] = np.where(track.mov_state.stat.data & ~track.mov_state.stat.mask,
																 self.get_grouptype('SLR25_RFF_STAT'),
																 self.get_grouptype('SLR25_RFF_MOV'))
				...
				elapsed = done - start
				logger.info("SLR25 RFF object retrieval is completed in " + str(elapsed) + " seconds")
...
if __name__ == '__main__':
		...
		conti, objects = manager_modules.calc('fillSLR25_RFF@aebs.fill', manager)
		print(objects)
