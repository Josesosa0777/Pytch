...
class cFill(iObjectFill):
		...
		def check(self):
				...
				return retest_aebs_events, time, egomotion

		def fill(self, retest_aebs_events, time, egomotion):
				# create object
				o = {}
				...
				for event in retest_aebs_events:
				...
				o["valid"] = mask
				o["label"] = np.array(["FLR25_AEB_RETEST_RESIM_%d" % id for id in o["id"]])

				o["type"] = np.where(~mask,
						self.get_grouptype('NONE_TYPE'),
						self.get_grouptype('FLR25_AEB_RETEST_RESIM'))
