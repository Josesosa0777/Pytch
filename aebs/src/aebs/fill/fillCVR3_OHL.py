...
DeviceNames = ("MRR1plus", ("ohl","ohy")), ("RadarFC",  ("ohy",))
SignalGroupsDict = {}
for dn, comps in DeviceNames:
  for comp in comps:
    SignalGroupsDict[(dn,comp)] = []
    for i in xrange(OHL_OBJ_NUM):
      SignalGroup = {}
      for Alias in Aliases:
        SignalGroup[Alias] = dn, '%s.ObjData_TC.OhlObj.i%d.%s' %(comp, i, Alias)
      SignalGroupsDict[(dn,comp)].append(SignalGroup)
...
class cFill(interface.iObjectFill):
  def check(self):
    ...
    for dn, comps in DeviceNames:
      for comp in comps:
        filtsglist.append(interface.Source._filterSignalGroups(SignalGroupsDict[(dn,comp)]))
  ...
  def fill(self, Groups):
    ...
    for i in xrange(OHL_OBJ_NUM):
      ...
      o["type"] = np.where( Stand_b==1,
                            self.get_grouptype('CVR3_OHL_STAT'),
                            self.get_grouptype('CVR3_OHL_MOV'))
                            
      o["label"] = "CVR3_OHL_%d"%i # OHL index as simple label 
