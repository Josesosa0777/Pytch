...
class cFill(iObjectFill):
		dep = 'fill_flc25_aebs_second_obj', 'calc_egomotion'

		def check(self):
				modules = self.get_modules()
				tracks = modules.fill("fill_flc25_aebs_second_obj")
		...
		def fill(self, tracks, egomotion):
				...
				for id, track in tracks.iteritems():
						...
						o["label"] = np.array( ["FLC25_AEBS_SECOND_OBJ_%d" %id for id in o["id"]] )
						...
						o["type"] = np.where(track.id.mask,
								...
								self.get_grouptype('FLC25_AEBS_SECOND_OBJ'))
...
if __name__ == '__main__':
		...
		time,objects = manager_modules.calc('fillFLC25_AEBS_SECOND_OBJ@aebs.fill', manager)
		print(objects)