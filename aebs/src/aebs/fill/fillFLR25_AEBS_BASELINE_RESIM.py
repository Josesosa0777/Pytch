...
class cFill(iObjectFill):
		dep = 'calc_aebs_resim2resim_output', 'calc_radar_egomotion-flr25'

		def check(self):
				modules = self.get_modules()
				baseline_aebs_events, retest_aebs_events, time, total_mileage = modules.fill("calc_aebs_resim2resim_output")
				egomotion = modules.fill("calc_radar_egomotion-flr25")
				return baseline_aebs_events, time, egomotion

		def fill(self, baseline_aebs_events, time, egomotion):
				# create object
				o = {}
				...
				for event in baseline_aebs_events:
				...
				o["valid"] = mask
				o["label"] = np.array(["FLR25_AEB_BASELINE_RESIM_%d" % id for id in o["id"]])

				o["type"] = np.where(~mask,
						self.get_grouptype('NONE_TYPE'),
						self.get_grouptype('FLR25_AEB_BASELINE_RESIM'))
				init_intervals = [(st, st + 1) for st, end in alive_intervals]
				o["init"] = intervalsToMask(init_intervals, len(time))
				o["color"] = [0, 255, 0]
					# colorByVelocity(egomotion.vx, o["vx"], [0, 255, 0], [255, 0, 0], [0, 0, 255])
				objects = [o]
				return time, objects
...
if __name__ == '__main__':
		...
		meas_path = r"C:\KBData\Measurement\fusion2.9_testing\aebs\delta_report\2021-07-13\mi5-5386__2021-07-13_05-25-02.mat"
		config, manager, manager_modules = init_dataeval(['-m', meas_path])
		conti, objects = manager_modules.calc('fillFLR25_AEBS_RESIM@aebs.fill', manager)
		print(conti)