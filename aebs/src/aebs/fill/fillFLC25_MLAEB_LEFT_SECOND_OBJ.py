...
class cFill(iObjectFill):
		dep = 'fill_flc25_mlaeb_left_second_obj', 'calc_egomotion'

		def check(self):
				modules = self.get_modules()
				tracks = modules.fill("fill_flc25_mlaeb_left_second_obj")
		...
		def fill(self, tracks, egomotion):
				...
				for id, track in tracks.iteritems():
						...
						o["label"] = np.array( ["FLC25_MLAEB_LEFT_SECOND_OBJ%d" %id for id in o["id"]] )
						...
						o["type"] = np.where(track.id.mask,
								...
								self.get_grouptype('FLC25_MLAEB_LEFT_SECOND_OBJ'))
