...
class cFill(iObjectFill):
		dep = 'fill_flc25_paebs_debug', 'calc_egomotion'

		def check(self):
				modules = self.get_modules()
				tracks = modules.fill("fill_flc25_paebs_debug")
		...
		def fill(self, tracks, egomotion):
				import time
				start = time.time()
				logger.info("FLC25 PAEBS DEBUG object retrieval is started, Please wait...")
				objects = []
				for id, track in tracks.iteritems():
					...
					o["label"] = np.full(len(o["id"]),"FLC25_PAEBS_DEBUG",dtype=object)
					...
					type_arr = np.full_like(track.motion_state.data,self.get_grouptype('FLC25_PAEBS_DEBUG_UNDEFINED'))
					type_arr[(track.motion_state.data == 4) & (track.obj_type.data == 3 ) & ~track.motion_state.mask] = self.get_grouptype('FLC25_PAEBS_DEBUG_PED_CROSS')
					type_arr[(track.motion_state.data != 4) & (track.obj_type.data == 3 ) & ~track.motion_state.mask] = self.get_grouptype('FLC25_PAEBS_DEBUG_PED_NOT_CROSS')
					type_arr[(track.motion_state.data == 4) & (track.obj_type.data == 5 ) & ~track.motion_state.mask] = self.get_grouptype('FLC25_PAEBS_DEBUG_BIC_CROSS')
					type_arr[(track.motion_state.data != 4) & (track.obj_type.data == 5 ) & ~track.motion_state.mask] = self.get_grouptype('FLC25_PAEBS_DEBUG_BIC_NOT_CROSS')
					...
					o["class_conf_paebs"] = track.class_conf.data
					o["lane_conf_paebs"] = track.lane_conf.data
					...
					o["prob_exist"] = track.prob_exist.data
					o["in_path"] = track.in_path.data
					o["vy_abs"] = track.vy_abs.data
					o["vy"] = track.vy.data
					o["vx"] = track.vx.data
					o['obj_type'] = track.obj_type.data
...
if __name__ == '__main__':
		...
		conti, objects = manager_modules.calc('fillFLC25_PAEBS_DEBUG@aebs.fill', manager)
		print(objects)
