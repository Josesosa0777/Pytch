...
class cFill(iObjectFill):
		dep = 'calc_aebs_resim_output', 'calc_radar_egomotion-flr25'

		def check(self):
				modules = self.get_modules()
				acc_ped_events,time,total_mileage = modules.fill("calc_aebs_resim_output")
		...
		def fill(self, acc_ped_events, time, egomotion):
				...
				o["valid"] = mask
				o["label"] = np.array(["FLR25_AEBS_RESIM_%d" % id for id in o["id"]])

				o["type"] = np.where(~mask,
						self.get_grouptype('NONE_TYPE'),
						self.get_grouptype('FLR25_AEB_RESIM'))
