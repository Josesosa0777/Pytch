...
logger = logging.getLogger('fillSLR25_RFB')

INVALID_ID = 255


class cFill(iObjectFill):
		dep = 'fill_slr25_rfb_tracks', 'calc_egomotion'

		def check(self):
				modules = self.get_modules()
				tracks, signals = modules.fill("fill_slr25_rfb_tracks")
		...
		def fill(self, tracks,signals, egomotion):
				import time
				start = time.time()
				logger.info("SLR25 RFB object retrieval is started, Please wait...")
				objects = []
				for id, track in tracks.iteritems():
						...
						if isinstance(id, int):
							...
							# o["label"] = np.array(
							# 				["SLR25_RFB_%d_%s" % (idx, obj_type) for idx, obj_type in zip(o["id"], object_type.data)])
							o["label"] = np.array(["SLR25_RFB" for idx, obj_type in zip(o["id"], object_type.data)])
							...
							o["type"] = np.where(track.mov_state.stat.data & ~track.mov_state.stat.mask,
																	 self.get_grouptype('SLR25_RFB_STAT'),
																	 self.get_grouptype('SLR25_RFB_MOV'))
							...
							o["obj_type_mapping"] = {
									...
									7: 'wide', 8: 'mirror', 9: 'multiple', 10: 'initialized'
							}

							o["custom_labels"] = {'video_conf': 'Probability Of Existence',}
							o['BSD4_LatDispMIORightSide_D0'] = tracks['BSD4_LatDispMIORightSide_D0']
							o['BSD4_LonDispMIORightSide_D0'] = tracks['BSD4_LonDispMIORightSide_D0']
							objects.append(o)
				done = time.time()
				elapsed = done - start
				logger.info("SLR25 RFB object retrieval is completed in " + str(elapsed) + " seconds")
...
if __name__ == '__main__':
		...
		meas_path = r"\\pu2w6474\shared-drive\measurements\SRR520_Kaan\mi5idextract__2021-10-28_12-24-02_Axiscamera1_0206300000.mat"
		config, manager, manager_modules = init_dataeval(['-m', meas_path])
		conti, objects = manager_modules.calc('fillSLR25_RFB@aebs.fill', manager)
		print(objects)
